...
class WorldClockCarousel:
    ...
    def __init__(self, cities: Iterable[tuple[str, float]]) -> None:
        ...
        if not city_list:
            raise ValueError("WorldClockCarousel needs at least one city.")
    ...
    @property
    def current(self) -> WorldClockNode:
    ...
    def shortest_offset_to_city(self, city_name: str) -> int:
        ...
        if clockwise_steps is None or counterclockwise_steps is None:
            raise ValueError(f"Unknown city name: {city_name!r}")
    ...
    def _steps_to_city(self, city_name: str, *, clockwise: bool) -> int | None:
        ...
        for step in range(self._size):
            ...
            if next_node is None:
                raise RuntimeError("WorldClockCarousel links are not valid.")
            node = next_node
        return None

    def rotate_next(self, steps: int = 1) -> WorldClockNode:
        return self._move(steps=steps, clockwise=True)

    def rotate_prev(self, steps: int = 1) -> WorldClockNode:
        return self._move(steps=steps, clockwise=False)

    def node_at_offset(self, offset: int) -> WorldClockNode:
        if offset == 0:
            return self._current
    ...
    def _move(self, *, steps: int, clockwise: bool) -> WorldClockNode:
        if steps < 0:
